...
impl Counter {
    ...
    fn sum(
        ...
        #[tool(description = "Suggest a movie")]
    fn suggest_movie(&self) -> Result<CallToolResult, McpError> {
        Ok(CallToolResult::success(vec![Content::text(
            "Pulp Fiction by Tarantino".to_string(),
        )]))
    }

     #[tool(description = "Guess the secret number and get feedback")]
    fn guess_number(
        &self,
        #[tool(param)]
        #[schemars(description = "Your guess for the secret number")]
        guess: i32,
    ) -> Result<CallToolResult, McpError> {
